...
const CartPage = () => {
  ...
  const handleCheckout = async () => {
    ...
    if (!addressId) return setError('Vui lòng thêm và chọn địa chỉ giao hàng.');
  ...
  if (orderSuccess) {
    return (
      <div className="grid min-h-[70vh] place-items-center bg-slate-50 px-4 py-16 dark:bg-slate-950">
        <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-8 text-center shadow-soft dark:border-slate-800 dark:bg-slate-900">
  ...
  return (
    <div className="min-h-screen bg-slate-50 py-10 dark:bg-slate-950">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {items.length === 0 ? (
          <div className="mx-auto max-w-2xl rounded-2xl border border-slate-200 bg-white p-12 text-center shadow-sm dark:border-slate-800 dark:bg-slate-900">
            <ShoppingBag className="mx-auto h-12 w-12 text-premium-700" />
          ...
          <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
            <div className="space-y-4 lg:col-span-2">
              {error && <div className="flex items-center gap-2 rounded-xl border border-red-100 bg-red-50 p-4 text-sm font-bold text-red-600"><AlertCircle className="h-4 w-4" /> {error}</div>}
              {items.map((item) => (
                <div key={item.variant_id} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                  <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
                    <img src={getImageUrl(item.image_url)} alt={item.name} className="h-24 w-24 flex-shrink-0 rounded-xl object-cover object-top bg-slate-100" />
                    ...
                    <div className="flex items-center justify-between gap-4 sm:justify-end">
                      <div className="flex items-center overflow-hidden rounded-xl border border-slate-200 dark:border-slate-700">
                        <button onClick={() => changeQuantity(item.variant_id, item.quantity - 1)} className="p-2 hover:bg-slate-50 dark:text-white"><Minus className="h-4 w-4" /></button>
                        ...
                        <button onClick={() => changeQuantity(item.variant_id, item.quantity + 1)} className="p-2 hover:bg-slate-50 dark:text-white"><Plus className="h-4 w-4" /></button>
                      </div>
                      ...
                      <button onClick={() => removeFromCart(item.variant_id)} className="rounded-md p-2 text-slate-400 hover:bg-red-50 hover:text-red-600"><Trash2 className="h-5 w-5" /></button>
                    </div>
                  </div>
                </div>
              ))}
              ...
              <div className="sticky top-24 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                ...
                {isAuthenticated ? (
                  <div className="space-y-3 py-5">
                    <select value={addressId} onChange={(e) => setAddressId(e.target.value)} className="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm dark:border-slate-700 dark:bg-slate-950 dark:text-white">
                    ...
                    {addresses.length === 0 && <Link to="/account/addresses" className="block text-sm font-bold text-premium-700">Thêm địa chỉ trước khi đặt hàng</Link>}
                    <textarea value={shippingNote} onChange={(e) => setShippingNote(e.target.value)} placeholder="Ghi chú giao hàng" className="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm dark:border-slate-700 dark:bg-slate-950 dark:text-white" />
                  </div>
                ) : (
                  <p className="py-5 text-sm text-slate-500">Bạn cần đăng nhập để chọn địa chỉ giao hàng.</p>
                )}
                ...
                <div className="space-y-3 border-t border-slate-100 pt-5 text-sm dark:border-slate-800">
                  ...
                  {shipping.estimated_days && <div className="text-xs font-bold text-premium-700">Dự kiến: {shipping.estimated_days}</div>}
                  <div className="flex justify-between border-t border-slate-100 pt-4 text-base dark:border-slate-800">
                    ...
                    <span className="font-black text-premium-800">{formatPrice(totalAmount + Number(shipping.shipping_fee || 0))}</span>
                  </div>
                </div>
