...
- Dùng MEMORY_CONTEXT để hiểu câu hỏi nối tiếp như "còn màu đen không", nhưng không bịa dữ liệu.
...
const buildCouponReply = (coupons) => {
  if (coupons.length === 0) return 'Hiện mình chưa thấy voucher khả dụng trong hệ thống. Bạn có thể vào trang Săn voucher để kiểm tra nhiệm vụ mới nhất.';
...
const buildPolicyReply = (policies) => {
  if (policies.length === 0) return 'Mình có thể hỗ trợ thông tin chung về giao hàng, thanh toán COD, voucher và đơn hàng. Với trường hợp cụ thể, bạn nên kiểm tra trong trang đơn hàng hoặc liên hệ shop.';
  return policies.slice(0, 2).map((policy) => policy.content).join(' ');
};

const buildLocalFallbackReply = (intent, context) => {
  if (intent === 'greeting') return 'Chào bạn, mình là trợ lý của LuxuryWear. Bạn muốn mình hỗ trợ tìm sản phẩm, xem voucher hay kiểm tra thông tin đơn hàng?';
  ...
  if (intent === 'size') {
    return context.products.length === 0
      ? 'Bạn cho mình thêm loại sản phẩm muốn mua, chiều cao, cân nặng và kiểu mặc thích rộng hay vừa nhé. Mình sẽ lọc lại sản phẩm phù hợp hơn.'
  ...
  if (intent === 'search') {
    if (context.products.length === 0) return 'Mình chưa thấy sản phẩm nào khớp đúng loại hoặc ngân sách bạn đưa ra. Bạn thử nới ngân sách, đổi màu, hoặc dùng từ khóa rộng hơn một chút nhé.';
  ...
  if (intent === 'styling') {
    if (context.products.length === 0) return 'Mình chưa tìm thấy sản phẩm đủ khớp để phối theo yêu cầu này. Bạn thử nới ngân sách hoặc mô tả rộng hơn, ví dụ “áo khoác đi chơi dưới 900k”.';
    const firstCategory = context.products[0]?.category || 'sản phẩm phù hợp';
...
export const chatWithStylist = async (req, res, next) => {
  try {
    ...
    const context = await retrieveAiContext({ message: cleanMessage, userId: req.user?.id, history });
    const messages = [
